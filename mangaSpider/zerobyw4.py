...
# index url
mangaIndexUrl = "https://www.gufengmh.com/manhua/yongzheheluku/"
...
if os.path.isfile(dirPath+mangaFileName):
    ...
    mangaIndexRawHtml = requests.get(mangaIndexUrl, verify=False, headers=headers, timeout=10)
    if(mangaIndexRawHtml.status_code == 200):
        soup = BeautifulSoup(mangaIndexRawHtml.content, 'html.parser')
        # process html
        mangaIndexHtml = soup.find("ul", {"id": "chapter-list-1"}).find_all('a')
    else:
        print('Oops, manga index page error.')
        exit()
    mangaStr = ""
    for mangaChapter in mangaIndexHtml:
        mangaStr += domain + mangaChapter['href'] + ":" + str(mangaChapter.get_text()).replace("\n", '') + "\n"
    ...
    for chapterLink in mangaList:
        ...
        while pageExist == 1:
            ...
            if imageUrl.status_code == 200:
                ...
                imageEnd = re.search(r'(jpg)|(png)', imageRawHtml.contents[0], re.IGNORECASE).span()[1]
                # print(imageDomain + imageRawHtml.contents[0][imageStart:imageEnd])
                # exit()
